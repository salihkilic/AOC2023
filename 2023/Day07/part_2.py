...
class Hand:
    ...
    def calculate_score(self) -> int:
        ...
        amount_of_jokers = self.cards.count("J")
        card_set.discard("J")
        if len(card_set) <= 1:
            return 7  # Five of a kind
        elif len(card_set) == 2:
            for card in card_set:
                # We still need to account for the jokers when counting cards
                if self.cards.count(card) + amount_of_jokers == 4:
                    return 6  # Four of a kind
            return 5  # Full house
        elif len(card_set) == 3:
            for card in card_set:
                if self.cards.count(card) + amount_of_jokers == 3:
